...
# Seed Example Nodes Script
# Usage: ./scripts/seed-nodes.sh
...
if ! mongosh "${MONGO_URI}" --quiet --eval "db.version()" &> /dev/null; then
    ...
    echo "  - Then run: MONGO_HOST=localhost ./scripts/seed-nodes.sh"
    exit 1
fi
...
mongosh "${MONGO_URI}" --quiet < "$(dirname "$0")/seed-example-nodes.js"
